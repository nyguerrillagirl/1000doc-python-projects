...
together as possible according to straight-line distance. In this example, the two junction boxes which are
...
are 162,817,812 and 431,825,988. After connecting them, since 162,817,812 is already connected to another junction box,
there is now a single circuit which contains three junction boxes and an additional 17 circuits which contain
...
Answer:
 ...
 --- Part Two ---
 The Elves were right; they definitely don't have enough extension cables. You'll need to keep connecting
 junction boxes together until they're all in one large circuit.

 Continuing the above example, the first connection which causes all of the junction boxes to form a single circuit
 is between the junction boxes at 216,146,977 and 117,168,530. The Elves need to know how far those junction
 boxes are from the wall so they can pick the right extension cable; multiplying the X coordinates of those two
 junction boxes (216 and 117) produces 25272.
...
def only_one_circuit(circuit_list):
    first_circuit = circuit_list[0]
    has_only_one_circuit = True
    for i in range(1, len(circuit_list)):
        if circuit_list[i] != first_circuit:
            has_only_one_circuit = False
            break

    return has_only_one_circuit
...
def main():
    ...
    threshold_point = None              # This is set when the circuit_list becomes the same value from 0..len(circuit_list)
    for _ in range(len(heap)):
        ...
        # check if there is only one circuit
        if only_one_circuit(circuit_list):
            threshold_point = [point_list[i], point_list[j]]
            break

    print(f"({threshold_point[0]},{threshold_point[1]})")
    result = threshold_point[0][0] * threshold_point[1][0]
