...
Decorating here will be easy: they can make their own decorations. What you really need is a way to get
further into the North Pole base while the elevators are offline.
...
If you can figure out which rolls of paper the forklifts can access, they'll spend less time looking and more
...
Note: The algorithm noted loops too many times over the same dataset. I know I can do it with one pass per
accessible paper roll determinations. But, the code would be uglier since give an (row,col) I could only process known
cells (cells above and to the left of (row, col), and then also in (row,col) I would have to update all cells that did
not know about (row,col) that would be all cells to above and to the left. ...see notes for more information.
'''
...
def remove_accessible_paper_rolls(grid, number_of_rows, number_of_cols):
    for row in range(number_of_rows):
        for col in range(number_of_cols):
            if grid[row][col][0] == ACCESSIBLE_PAPER_ROLL:
                grid[row][col][0] = EMPTY_DOT
                grid[row][col][1] = 0

def process_for_adjacent_paper_rolls(grid, number_of_rows, number_of_cols):
    for row in range(number_of_rows):
        for col in range(number_of_cols):
            # initialize the adjacency count to 0 for this cell
            grid[row][col][1] = 0
...
def main():
    ...
    total_accessible_paper_roll_count = 0
    while True:
        ...
        total_accessible_paper_roll_count += accessible_paper_roll_count
        if accessible_paper_roll_count == 0:
            break
        # clear
        remove_accessible_paper_rolls(grid, number_of_rows, number_of_cols)

    print(f"total_accessible_paper_roll_count:  {total_accessible_paper_roll_count}")
